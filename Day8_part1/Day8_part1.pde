...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day8_part1\\data\\myData");
...
public class SegDecoder
{
  ...
  {
    int i=0;
    int l=outParts.length;
    int total=0;
    
    for (i=0;i<l;i++)
    {
      if (outParts[i].length()==2||outParts[i].length()==4||outParts[i].length()==3||outParts[i].length()==7)
      {
        total++;
      }
    }
