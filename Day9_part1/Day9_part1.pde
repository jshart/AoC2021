...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day9_part1\\data\\mydata");
...
void setup() {
  ...
  x=input.lines.size();
  y=input.lines.get(0).length();
  
  println("Init map size:"+x+","+y);
  dMap=new int[x][y];
  
  for (i=0;i<x;i++) // loop through each "line" in the input
  {
    for (j=0;j<y;j++) // loop through each "char" in the line
    {
