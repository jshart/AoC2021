...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day7_part1\\data\\example");
...
int maxList=20;
Crab[] crabList = new Crab[maxList];
Maximum maxH=new Maximum();
Minimum minH=new Minimum();
...
void setup() {
  ...
  bucketCrabs(input.lines.get(0));
  printCrabs();
  
  long tf=0;
  long fl=0;
  long ifl=0;
  // brute force each fuel level
  for (test=0;test<=maxH.value;test++)
  {
    tf=0;
    
    // check each crab to calculate what this level would require for fuel
    for (j=0;j<maxList;j++)
    {
      // Walk through each crab and check the height - is there any crab at this height?
      if (crabList[j]!=null)
      {
        //print("testH:"+test+" Cnt:"+crabList[j].count+" H:"+j);

        ifl=abs(j-test);
        fl=(ifl*crabList[j].count);
        //println("...IFL:"+ifl+" Total Fl:"+fl);
        tf+=fl;
      }
    }
    println("...tf:"+tf);
...
void bucketCrabs(String s)
{
  ...
  for (i=0;i<l;i++)
  {
    h=Integer.parseInt(temp[i]);
    maxH.set(h);
    t+=h;
    
    if (crabList[h]==null)
    {
      Crab tempCrab = new Crab();
      crabList[h]=tempCrab;
    }
...
void printCrabs()
{
  ...
  for (i=0;i<maxList;i++)
  {
    if (crabList[i]!=null)
    {
      println("H:"+i+" C:"+crabList[i].count);
    }
  }
}
