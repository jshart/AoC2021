...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day18_part2\\data\\mydata");
...
void setup() {
  ...
  for (j=0;j<input.lines.size();j++)
  {
    for (i=0;i<input.lines.size();i++)
    {
      currentString=input.lines.get(j);
      ...
      t=currentSf.left.mag();
      println("FINAL MAG:"+t);
      max.set(t);
      println("CURRENT MAX:"+max.value);
    }
  }
}
