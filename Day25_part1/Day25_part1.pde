...
public class Sea
{
  ...
  public void checkCanMove(int t)
  {
    int x=0,y=0;
    ...
    for (x=0;x<maxX;x++)
    {
      for (y=0;y<maxY;y++)
      {
        // is this the right Type of flock?
        if (matrix[x][y]==t)
        {
          // setup the can move matrix based on if
          // there is a free space this one can
          // move into
          canMove[x][y]=freeSpace(x,y);
        }
      }
    }
  }
