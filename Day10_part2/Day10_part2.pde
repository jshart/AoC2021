...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day10_part2\\data\\mydata");
...
long parseLine(String s)
{
  ...
  println();
  println("*** BEGIN parsing;"+s);

  for (i=0; i<l; i++)
  {
    inspected=s.charAt(i);
    ...
    switch (inspected)
    {
      ...
      case ')':
      case '>':
        expected=whatToExpect(stack.charAt(stack.length()-1));
        ...
        if (inspected!=expected)
        {
          ...
          println("|-- ERROR, found: "+inspected+" was expecting: "+expected);
          //return(score);
          return(0);
        }
        else
        {
          println("|-- GOOD, found: "+inspected+" was expecting: "+expected);
          score=0;
        }
        break;
    }
  }
  print("*** COMPELTE, stack remaining;");
  print(stack);
  print(" === ");
  score2=missingStack(stack);
  
  return(score2);
}

long missingStack(String s)
{
  int i=0;
  int l=s.length();
  ...
  // calculate score - I know we could do this above,
  // but I like to keep it seperate to make it more
  // readable
  long score=0;
