...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2021\\Day11_part1\\data\\mydata");
...
int maxX;
int maxY;
int stepCount=1; // init to 1, because the "loaded" state is considered step 0
int drawStep=0;
...
void setup() {
  ...
  flashMask=new boolean[maxX][maxY];
...
void updateMatrixBy1()
{
  int x=0, y=0;
  for (x=0;x<maxX;x++) // loop through each "line" in the input
  {
    for (y=0;y<maxY;y++) // loop through each "char" in the line
    {
      // update all light levels
      dMap[x][y]++;
    }
  }
}
...
void flashMatrix()
{
  ...
  do
  {
    ...
    for (x=0;x<maxX;x++) // loop through each "line" in the input
    {
      for (y=0;y<maxY;y++) // loop through each "char" in the line
      {
        if (dMap[x][y]>9 && flashMask[x][y]==false)
        {
          // cause a flash
          causeFlash(x,y);
          flashMask[x][y]=true;
...
void draw() {  
  ...
  println("/----------");
  println("|-- STEP:"+stepCount+" MODE:"+drawStep);

  switch (drawStep)
  {
    case 0:
      println("|-- UPDATING --- FLASH COUNT SO FAR ="+flashCount);
      updateMatrixBy1();
      drawStep++;
      break;
    case 1:
      println("|-- FLASHING --- FLASH COUNT SO FAR ="+flashCount);
      flashMatrix();
      
      allFlash=didAllFlash();
      if (allFlash==true)
      {
        println("### ALL FLASHED at step:"+stepCount);
      }
      drawStep++;
      break;
    case 2:
      println("|-- CLEARING --- FLASH COUNT SO FAR ="+flashCount);
      clearFlash();
      drawStep++;
      //break;
    default:
